...
const MemoryService = {
    ...
    generateHistorySummary: function(entries) {
      ...
      const healthProgression = entries.map(entry => ({
      ...
      if (entries.length > 0) {
        const firstDate = entries[0].timestamp instanceof Date ? entries[0].timestamp.toLocaleDateString() : entries[0].timestamp;
        const lastDate = entries[entries.length - 1].timestamp instanceof Date ? entries[entries.length - 1].timestamp.toLocaleDateString() : entries[entries.length - 1].timestamp;
